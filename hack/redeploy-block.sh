...
#   ./hack/redeploy-block.sh --filesystem             # deploy Filesystem-mode SBR instead of Block
...
#
# Env: NAMESPACE, STORAGE_CLASS, VOLUME_MODE (Block|Filesystem, default Block), SKIP_BUILD, FORCE_BUILD,
#      BUILD_MODE (git|binary), GIT_URI, GIT_REF, SKIP_VERIFY, SKIP_WIRING,
#      DETECT_ONLY (default true), CONFIG_NAME (default sbr-block), DELETE_NAMESPACE.
...
# Mirror build-in-cluster.sh's defaults here (and export them) so the source fingerprint we compute
# is derived from the exact uri/ref that build-in-cluster.sh will clone.
GIT_URI="${GIT_URI:-https://github.com/medik8s/storage-based-remediation}"
GIT_REF="${GIT_REF:-main}"
export GIT_URI GIT_REF
...
# --- source fingerprinting (skip rebuilds when the source is unchanged) ---
# We stamp a fingerprint of the built source onto the sbr-operator/sbr-agent ImageStreams after a
# successful build, then compare it on the next run. Idempotent: an unchanged source is a no-op.
FINGERPRINT_JSONPATH='{.metadata.annotations.sbr\.medik8s\.io/source-fingerprint}'

compute_source_fingerprint() {
    if [[ "$BUILD_MODE" == "git" ]]; then
        # The resolved remote commit SHA for GIT_URI@GIT_REF. A ref that is already a full SHA
        # yields no ls-remote match, so fall back to the ref verbatim.
        local sha
        sha="$(git ls-remote "$GIT_URI" "$GIT_REF" 2>/dev/null | awk 'NR==1{print $1}')"
        [[ -z "$sha" ]] && sha="$GIT_REF"
        printf 'git:%s@%s' "$GIT_URI" "$sha"
    else
        # binary mode builds the working tree (committed state + tracked edits + untracked files),
        # so hash all three. git hash-object avoids shasum/sha256sum portability differences.
        local content
        content="$( { git -C "$REPO_ROOT" rev-parse HEAD 2>/dev/null || echo no-head
                      git -C "$REPO_ROOT" diff HEAD 2>/dev/null
                      git -C "$REPO_ROOT" ls-files --others --exclude-standard -z 2>/dev/null \
                          | xargs -0 -r cat 2>/dev/null; } | git hash-object --stdin)"
        printf 'binary:%s' "$content"
    fi
}

stored_fingerprint() {
    oc get imagestream "$1" -n "$NAMESPACE" -o jsonpath="$FINGERPRINT_JSONPATH" 2>/dev/null
}

record_fingerprint() {
    oc annotate imagestream sbr-operator sbr-agent -n "$NAMESPACE" \
        "sbr.medik8s.io/source-fingerprint=$1" --overwrite >/dev/null 2>&1 || true
}

# True only if both istags exist and both carry the current fingerprint (and no forced rebuild).
images_up_to_date() {
    local fp="$1"
    [[ "$FORCE_BUILD" == "true" ]] && return 1
    oc get istag sbr-operator:latest -n "$NAMESPACE" >/dev/null 2>&1 || return 1
    oc get istag sbr-agent:latest    -n "$NAMESPACE" >/dev/null 2>&1 || return 1
    [[ "$(stored_fingerprint sbr-operator)" == "$fp" ]] || return 1
    [[ "$(stored_fingerprint sbr-agent)"    == "$fp" ]] || return 1
    return 0
}
...
OPTIONS:
  ...
  --force-build           Always rebuild, even if the source fingerprint is unchanged.
                          (By default the build is skipped when the source — git ref or
                          local working tree — matches the last build's fingerprint.)
  ...
  --block                 Deploy in Block mode (raw-block RWX device). This is the DEFAULT.
  --filesystem            Deploy in Filesystem mode (shared-filesystem RWX). Auto-detects a
                          CephFS/Portworx SC and skips the block-specific verification step.
  --volume-mode <Block|Filesystem>
                          Explicit form of --block / --filesystem.
...
ENVIRONMENT (equivalents to the flags above):
  NAMESPACE, STORAGE_CLASS, VOLUME_MODE (Block|Filesystem), SKIP_BUILD, FORCE_BUILD,
  BUILD_MODE (git|binary), GIT_URI, GIT_REF, SKIP_VERIFY, SKIP_WIRING,
  DETECT_ONLY, CONFIG_NAME, DELETE_NAMESPACE.

EXAMPLES:
  ./hack/redeploy-block.sh
  ./hack/redeploy-block.sh --filesystem
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --filesystem)       VOLUME_MODE="Filesystem"; shift ;;
        --block)            VOLUME_MODE="Block"; shift ;;
        --volume-mode|--volume-mode=*)
            if [[ "$1" == *=* ]]; then VOLUME_MODE="${1#*=}"; else shift; VOLUME_MODE="$1"; fi; shift ;;
...
case "$VOLUME_MODE" in
    Block|Filesystem) ;;
    *) fatal "invalid VOLUME_MODE '$VOLUME_MODE' (expected Block or Filesystem)" ;;
esac
...
# --- 2. storage: find an RWX StorageClass for the selected volume mode ---
# Block      -> raw-block RWX (Ceph RBD, or Portworx pxd).
# Filesystem -> shared-filesystem RWX (CephFS, or Portworx pxd which auto-promotes RWX+FS to sharedv4/NFS).
step "2/6 Storage ($VOLUME_MODE mode)"
find_block_sc() {
...
find_fs_sc() {
    oc get storageclass -o jsonpath='{range .items[*]}{.metadata.name}{"|"}{.provisioner}{"\n"}{end}' 2>/dev/null \
        | awk -F'|' '$2=="cephfs.csi.ceph.com" || $2=="openshift-storage.cephfs.csi.ceph.com" || $2=="pxd.portworx.com" {print $1; exit}'
}
if [[ -z "$STORAGE_CLASS" ]]; then
    if [[ "$VOLUME_MODE" == "Filesystem" ]]; then STORAGE_CLASS="$(find_fs_sc || true)"; else STORAGE_CLASS="$(find_block_sc || true)"; fi
fi
if [[ -z "$STORAGE_CLASS" ]]; then
    if [[ "$VOLUME_MODE" == "Filesystem" ]]; then
        fatal "no RWX-filesystem StorageClass found (supported provisioners: cephfs.csi.ceph.com, openshift-storage.cephfs.csi.ceph.com, pxd.portworx.com).
        Install ODF (for CephFS, e.g. ocs-storagecluster-cephfs) or Portworx (e.g. portworx-replica-two), then re-run."
    else
...
ok "Using $VOLUME_MODE StorageClass: $STORAGE_CLASS"
...
elif images_up_to_date "$FINGERPRINT"; then
    info "Source unchanged since last build ($BUILD_MODE fingerprint matches) — skipping rebuild"
    info "Force a rebuild with FORCE_BUILD=true / --force-build"
else
...
step "5/6 Verify $VOLUME_MODE mode"
if [[ "$VOLUME_MODE" == "Filesystem" ]]; then
    info "Filesystem mode — skipping block-mode verification (testing_operator_block_mode.sh is block-specific)"
...
if [[ "$SKIP_WIRING" == "true" ]]; then
    ...
    info "Applying $VOLUME_MODE SBRConfig '$CONFIG_NAME' (detectOnly=${DETECT_ONLY}) ..."
...
metadata:
  ...
  namespace: ${NAMESPACE}
spec:
  sharedStorageVolumeMode: ${VOLUME_MODE}
...
echo -e "\n${GREEN}=== SBR ${VOLUME_MODE}-mode redeploy complete ===${NC}" >&2
info "Namespace:      $NAMESPACE"
info "$VOLUME_MODE SC:   $STORAGE_CLASS"
info "SBRConfig:      $CONFIG_NAME ($VOLUME_MODE, detectOnly=${DETECT_ONLY})"
if [[ "$VOLUME_MODE" == "Filesystem" ]]; then
    warn "Known limitation: on Portworx sharedv4/NFS the agent on the NFS *server* node may
        CrashLoop on the O_DIRECT pre-flight r/w test (unaligned O_DIRECT EINVALs on the
        server's local ext4; NFS clients tolerate it). See finding-sbr-filesystem-odirect-einval."
fi
