...
#
# Two source modes:
#   git (default) — the cluster clones a repo + ref. Defaults to medik8s/storage-based-remediation@main.
#   binary (--binary) — upload the local working tree (builds exactly what is checked out,
#                       including uncommitted changes).
#
# Usage:
#   ./hack/build-in-cluster.sh [--namespace NS]                        # git, medik8s@main default
