...
EXISTING_CSV="$(oc -n "$PX_NAMESPACE" get csv -o name 2>/dev/null | grep -i portworx | head -1 || true)"
if [[ -n "$EXISTING_CSV" ]] \
  ...
  PX_PKG="$(oc get packagemanifests -o jsonpath='{range .items[*]}{.metadata.name}{"\n"}{end}' 2>/dev/null \
            | grep -i portworx | grep -iv essential | head -1 || true)"
...
if ! grep -q 'AWS_ACCESS_KEY_ID' "$SPEC_FILE"; then
  warn "spec $SPEC_FILE has no AWS credentials env block."
  cat <<'EOF' >&2
...
EOF
  die "add the spec env block, then re-run"
fi

# Point the aws-key/aws-secret secretKeyRefs at OUR secret ($SECRET_NAME).
if grep -q "$SECRET_NAME" "$SPEC_FILE"; then
  echo "  spec already references secret $SECRET_NAME"
elif confirm "rewrite secretKeyRef name in $SPEC_FILE to $SECRET_NAME"; then
  if $DRY_RUN; then
    echo "  [dry-run] would set secretKeyRef name -> $SECRET_NAME in $SPEC_FILE"
  else
    tmp="$(mktemp)"
    sed -E "/aws-key|aws-secret/ s#(secretKeyRef:[[:space:]]*\{[[:space:]]*name:[[:space:]]*)[^,}[:space:]]+#\1${SECRET_NAME}#g" \
      "$SPEC_FILE" > "$tmp" && mv "$tmp" "$SPEC_FILE"
    grep -q "$SECRET_NAME" "$SPEC_FILE" || die "failed to rewrite secretKeyRef name in $SPEC_FILE"
    echo "  set secretKeyRef name -> $SECRET_NAME"
  fi
fi
