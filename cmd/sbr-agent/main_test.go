...
var _ = Describe("Fence flow with real SBR agent", func() {
	...
	setupFenceFlowBase := func() (tmpDir, sbrPath, fencePath string, worker1ID, worker2ID uint16) {
		...
		// The remediation reconciler gates fencing on StorageValidation.ConcurrentWriteable being true on the owning
		// StorageBasedRemediationConfig
		By("Creating a StorageBasedRemediationConfig with StorageValidation.ConcurrentWriteable=true for fencing to be gated on")
